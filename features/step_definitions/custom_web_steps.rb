...
#Given /^I have user named "([^"]*)" and email "([^"]*)" and password "([^"]*)" and system_role (\d+)$/ do |name, email, password, system_role|
...
When /^I should see "([^"]*)" button/ do |name|
...
When /^I have album name "([^"]*)" and user_id (\d+) and share_type (\d+)$/ do |name, user_id, share_type|
  @album = Album.create!(:name => name, :user_id => user_id, :share_type => share_type)
end

When /^I have group user_id (\d+) and name "([^"]*)" and description "([^"]*)" and is_public (\d+)$/ do |user_id, name, description, is_public|
  @disc_group = DiscussionGroup.new(:user_id => @user.id, :name => name, :description => description, :is_public => is_public)
  @disc_group.save
  @disc_group_users = DiscussionGroupUser.new(:discussion_group_id => @disc_group.id, :user_id => @disc_group.user_id, :is_member => 1 )
  @disc_group_users.save
end

When /^I have public groups user_id (\d+) and name "([^"]*)" and description "([^"]*)" and is_public (\d+)$/ do |user_id, name, description, is_public|
  @disc_group = DiscussionGroup.new(:user_id => user_id, :name => name, :description => description, :is_public => is_public)
  @disc_group.save
end

When /^I join public groups$/ do 
  @disc_group_users = DiscussionGroupUser.create!(:discussion_group_id => @disc_group.id, :user_id => @user.id, :is_member => 1 )
end

When /^I have user named "([^"]*)" and email "([^"]*)" and password "([^"]*)" and system_role (\d+)$/ do |name, email, password, system_role|
  @user = User.create!(:username => name, :email => email, :encrypted_password => password, :system_role => system_role)
  @user_detail = @user.build_user_detail
  @user.activate
end

When /^I have public groups for joining user_id (\d+) and name "([^"]*)" and description "([^"]*)" and is_public (\d+)$/ do |user_id, name, description, is_public|
  @disc_group = DiscussionGroup.new(:user_id => @user.id, :name => name, :description => description, :is_public => is_public)
  @disc_group.save
  @all_disc_group = DiscussionGroup.find(:first, :conditions => ['user_id=? and is_public=?', @user.id, is_public])

end

When /^I am member of public groups user_id (\d+) and is_member (\d+)$/ do |user_id, is_member|
  @disc_group_users = DiscussionGroupUser.create!(:discussion_group_id => @all_disc_group.id, :user_id => user_id, :is_member => is_member )
end

When /^I leave public groups is_member (\d+)$/ do |is_member|
  @disc_group_users = DiscussionGroupUser.find(:first, :conditions =>['discussion_group_id =? and user_id=? and is_member=?', @disc_group.id, @user.id, is_member])
  @disc_group_users.destroy
end
