class DiscussionsController < ApplicationController
  ...
  def create_discussion
    ...
    @attachments.each_with_index do |at,i|
      disc_att = DiscussionAttachment.new(:file=>at,:description=>@attachment_desc[i],:discussion_id=>@lastest_discussion[0].id)
      disc_att.save
      if !disc_att.save
        @message = activerecord_error_list(disc_att.errors)
        raise ActiveRecord::Rollback, "to roll back if file is not acceptable"
      end
    end if !@attachment_desc.blank?
    @groups.each do |grp|
      disc_grp_disc = DiscussionGroupDiscussion.new(:discussion_group_id=>grp.to_i,:discussion_id=>@lastest_discussion[0].id)
      disc_grp_disc.save
  ...
  def update_comment_responses
    ...
    if @comments.user_id
      if @comments.update_attributes(:comment => params[:response_comment][:comment])
        ...
        @comments.discussion.users.each do |discussion_poster|
          Notifier.mail_to_user_on_discussion(@comments,discussion_poster).deliver if @comments.user_id != @login_user.id
        end
  ...
  def all_discussion
    @groups = @login_user.all_public_private_groups
    @most_pop_discussions = Discussion.most_popular((Time.now - 30.day), Time.now,@login_user.id,1)
  end
  ...
  def load_discussion_as_per_time
    ...
    @most_pop_discussions = Discussion.most_popular(start_time, Time.now, @login_user.id, page, params[:order_by]=="ratings", group_id)
  ...
  def load_feeds_by_group
    ...
    to = Time.now
    @feeds  = @login_user.feed(from, to, params[:id].to_i==0 ? nil : params[:id])
