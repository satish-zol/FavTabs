class DiscussionGroupsController < ApplicationController
  ...
  def delete_picture_comment
    ...
    if @pic.user_id==@login_user.id or @comm.user_id==@login_user.id
      if @comm.destroy
        respond_to do |format|
          format.js
        end
  ...
  def delete_picture
    @pic = Image.find(params[:id])
    if @login_user.is_member_of_discussion_group(@pic.imageable_id) && (@pic.user_id==@login_user.id ||  @pic.imageable.user_id == @login_user.id)
      if @pic.destroy
        render :text=>"Success"
      else
        render :text=>"Fail"
