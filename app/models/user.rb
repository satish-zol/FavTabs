class User < ActiveRecord::Base
  ...
  has_many :discussion_groups, :dependent => :destroy
  ...
  def add_discussion?(discussion)
    return self.discussions.where("discussion = ?", discussion[:discussion])[0] if self.discussions.where("discussion = ?", discussion[:discussion]).present?
  ...
  def find_public_groups(search_str, page=1)
    ...
    DiscussionGroup.find_by_sql(["SELECT DISTINCT dg.* FROM (discussion_groups dg) WHERE (dg.is_public=? #{query=='' ? '' : 'AND '+query}) LIMIT ? OFFSET ?",true, *where_array, per_page, off]);
  end
  ...
  def is_member_of_discussion_group(discussion_group_id)
    DiscussionGroupUser.find(:first, :conditions => ['discussion_group_id=? and user_id=? and is_member=?', discussion_group_id, self.id, true]).present?
  end
  ...
  def all_public_private_groups
    DiscussionGroup.find_by_sql(["Select dg.* from (discussion_groups dg LEFT JOIN discussion_group_users dgu ON dg.id=dgu.discussion_group_id ) where ((dg.is_public=? OR dg.is_public=?) AND dgu.user_id=? AND dgu.is_member=?) ORDER BY dg.name", true, false, self.id, true ])
  end
